...
import { dartPlatformName, flutterExtensionIdentifier, flutterPath, HAS_LAST_DEBUG_CONFIG, isWin, platformDisplayName } from "../shared/constants";
...
export function activate(context: vs.ExtensionContext, isRestart: boolean = false) {
	...
	if (!isUsingLsp) {
		context.subscriptions.push(vs.languages.registerDocumentHighlightProvider(activeFileFilters, new DartDocumentHighlightProvider(analyzer)));
	...
	const connectedSetup = analyzer.registerForServerConnected((sc) => {
		...
		const documentSymbolProvider = isUsingLsp ? undefined : new DartDocumentSymbolProvider(logger, analyzer);
